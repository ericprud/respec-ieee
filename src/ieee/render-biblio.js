...
import { addId, getIntlData, showError } from "../core/utils.js";
...
/** @param {Conf} conf */
export function run(conf) { debugger
...
function stringifyReference(ref) {
  ...
  let title = `<cite>${ref.title}</cite>`;

  output += ref.href ? `<a href="${ref.href}">${title}</a>. ` : `${title}. `;
